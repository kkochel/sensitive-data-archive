...
# Create database
psql -v ON_ERROR_STOP=1 --username postgres --dbname postgres --set db="${POSTGRES_DB:-fo-db}" <<-'EOSQL'
	SET TIME ZONE 'UTC';
	CREATE DATABASE :"db" ;
EOSQL
...
for f in docker-entrypoint-initdb.d/*; do
	echo "$0: running $f"
	echo
	psql -v ON_ERROR_STOP=1 --username postgres --dbname "${POSTGRES_DB:-fo-db}" -f "$f"
	echo
done
